<%@ page language="java" contentType="text/html; charset=ISO-8859-1" pageEncoding="ISO-8859-1"%>
<%@ taglib prefix="f"  uri="http://java.sun.com/jsf/core"%>
<%@ taglib prefix="h"  uri="http://java.sun.com/jsf/html"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
	<head>
	<meta http-equiv="Content-Type" content="text/html; charset=ISO-8859-1">
	<title>Solar Calculator Tool-&gt; Solar Power Details</title>
	<LINK href="<%=request.getContextPath()%>/css/greenhat_style.css" rel="stylesheet" type="text/css">
	    <style type="text/css">
	        td{
	        	width: 300px;
	            border-width: 0px;}
	        
	        table {	width: 450px;}
	    </style>
	</head>
	<body>
	 	<h4 align = "center"> Provide the following details: </h4>
		<f:view>
		    <h:form styleClass="input">
		    <table align="center">
			<tr>
			<td>
				<fieldset>
				<legend style = "align: center"> Equipment Details</legend>
				<table align="center">
					<tr>
						<td style="width: 250px;"><h:outputText value="Cost"></h:outputText></td>
						<td><h:inputText value="#{equipment.cost}"></h:inputText></td>
					</tr>
					
					<tr>
						<td><h:outputText value="System Size (KWh):"></h:outputText></td>
						<td><h:inputText value="#{equipment.size}" id="equipmentSize"></h:inputText></td>
					</tr>
					<tr>
						<td><h:outputText value="Inverter Efficiency (%):"></h:outputText></td>
						<td><h:inputText value="#{inverter.efficiency}" id="inverterEfficiency"></h:inputText></td>
					</tr>
					<!-- <tr>
						<td style="width: 250px;"><h:outputText value="Number of Panels"></h:outputText></td>
						<td>
							<h:selectOneMenu validatorMessage="required">
		        				<f:selectItems value="#{calculatorController.panels}" />
		      				</h:selectOneMenu>
						</td>
					</tr> -->
				</table>
				</fieldset>
				<p />
				<fieldset>
				<legend style = "align: center"> Roof Details</legend>
				<table align="center">
					<tr>
						<td><h:outputText value="Roof Size Width (cm):"></h:outputText></td>
						<td><h:inputText value="#{roof.width}" id="width"></h:inputText></td>
					</tr>
					<tr>
						<td><h:outputText value="Roof Size Height (cm):"></h:outputText></td>
						<td><h:inputText value="#{roof.height}" id="height"></h:inputText></td>
					</tr>
					<tr>
						<td><h:outputText value="Percentage on North Roof:"></h:outputText></td>
						<td><h:inputText value="#{roof.percentageNorth}" id="roofPercentageNorth"></h:inputText></td>
					</tr>
					<tr>
						<td><h:outputText value="Percentage on West Roof:"></h:outputText></td>
						<td><h:inputText value="#{roof.percentageWest}" id="roofPercentageWest"></h:inputText></td>
					</tr>
					<tr>
						<td><h:outputText value="Efficiency Loss (North Roof):"></h:outputText></td>
						<td><h:inputText value="#{roof.efficiencyLossNorth}" id="roofEfficiencyLossNorth"></h:inputText></td>
					</tr>
					<tr>
						<td><h:outputText value="Efficiency Loss (West Roof):"></h:outputText></td>
						<td><h:inputText value="#{roof.efficiencyLossWest}" id="roofEfficiencyLossWest"></h:inputText></td>
					</tr>
					
				</table>
				</fieldset>
				<p />
				<fieldset>
				<legend style = "align: center"> Sunlight Details</legend>
				<table align="center">
					<tr>
						<td><h:outputText value="Average Daily Hours of Sunlight:"></h:outputText></td>
						<td><h:inputText value="#{location.sunLightHours}" id="locationSunlightHours"></h:inputText></td>
					</tr>
				</table>
				</fieldset>
				<p />
				<fieldset>
				<legend style = "align: center"> Average Energy Usage Per Details</legend>
				<table align="left">
					<tr>
						<td><h:outputText value="Day:"></h:outputText></td>
						<td><h:inputText value="#{electricityUsage.dailyAverageUsage}" id="dailyAverageUsage"></h:inputText></td>
					</tr>
					<tr>
						<td><h:outputText value="DayTime Hour:"></h:outputText></td>
						<td><h:inputText value="#{electricityUsage.dayTimeHourlyUsage}" id="dayTimeHourlyUsage"></h:inputText></td>
					</tr>
				</table>
				</fieldset>
				<p />
				<fieldset>
				<legend style = "align: center"> Tariffs</legend>
				<table align="left">
					<tr>
						<td><h:outputText value="Tariff 11 Fee"></h:outputText></td>
						<td><h:inputText value="#{tariff.tariff11Fee}"></h:inputText></td>
					</tr>
				</table>
				</fieldset>
				<p />
				<fieldset> 
				<table>
					<tr>
						<td></td>
						<td align="right">
							<h:commandButton action="#{calculatorController.calculateSolarPower}" value="Calculate"></h:commandButton>
							<h:commandButton action="#{calculatorController.saveCalculation}" value="Save"></h:commandButton>
						</td>
					</tr>
				</table>
				</fieldset>
			</td>
			</tr>
			</table>
			</h:form>
		</f:view>
	 </body>
 </html>